...
def extract_feat(model, sound_input, fname,config):
    ...
    for idx in xrange(layer_min, layer_max):
        ...
        if config.is_save:
            np.save(os.path.join(config.outpath, fname.strip('.wav')+'_tf_fea{}.npy'.format( \
...
if __name__ == '__main__':
    ...
    if args.phase == 'demo':
        ...
        names = []
        for fil in files:
                names.append(fil.split('/')[-1].strip('.wav\n'))
    ...
    with tf.Session(config=sess_config) as session:
        ...
        for sound_sample in sound_samples:
            output = extract_feat(model, sound_sample,names[c],args)
            c+=1
